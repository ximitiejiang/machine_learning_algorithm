...
class SVMReg(BaseModel):
    ...
    def calc_kernel(self, train_x, kernel_option):
        ...
        for i in range(m):
            kernel_matrix[:, i] = self.cal_kernel_value(train_x, train_x[i, :], kernel_option)
        return kernel_matrix
    
    def cal_kernel_value(self, train_x, train_x_i, kernel_option):
        '''样本之间的核函数的值
        ...
        if kernel_type == 'rbf': # rbf核函数
            ...
            if sigma == 0:
                sigma = 1.0
            for i in range(m):
                diff = train_x[i, :] - train_x_i   # (2,) - (2,) = (2,)
    ...
    def cal_error(self, alpha_k):
        ...
        '''
        output_k = float(np.multiply(self.alphas, self.train_y).T * self.kernel_mat[:, alpha_k] + self.b)
    ...
    def svm_predict(self, test_sample_x):
        '''利用SVM模型对每一个样本进行预测
        input:  svm:SVM模型
        ...
        '''
        # 1、计算核函数矩阵
        kernel_value = self.cal_kernel_value(self.train_x, test_sample_x, self.kernel_opt)
        # 2、计算预测值
