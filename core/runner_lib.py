...
def get_batch_processor(cfg_model):
    if cfg_model.type == 'classifier':
...
class Runner():
    ...
    def __init__(self, cfg_path,):
        ...
        #batch处理
        self.batch_processor = get_batch_processor(cfg.model)
        #创建数据集
        ...
        if torch.cuda.is_available and cfg.gpus:
            ...
            logger.info('Training in GPU!')
        else:
            logger.info('Training in CPU!)
