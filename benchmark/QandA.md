...
1. 什么是模型的正则化：
    ...
    - 模型如果过拟合，说明模型过度匹配训练数据，也就是权值过度更新，或者说w过大，模型越复杂。
    ...
    - 正则化实施的特点：虽然正则化是作为惩罚加到损失函数，然后通过梯度反向传播影响每一个w的更新。
      但实际实施的便捷方式是直接在每个w的优化器中，对该w的grad添加正则项梯度weight_decay*W (这是l2正则化梯度表达式)。
      这样实施的好处是，每一个w的正则是分别进行的，且跟这个w的梯度直接相关。  

2. 两种正则化的区别？
    - l1正则化：采用所有权值w的绝对值之和作为正则项
    - l2正则化：采用
