...
<br>输入：训练数据集T
<br>输出：分类器G(x)
1. 初始化权值分布w1 = (w11,w12,..w1N), w1i=1/N，其中1N代表第1次权值，N个样本，用1/N作为每个权值的初始化值
2. 

**Q. ada boost是否可以采用其他分类器？且为什么一定要采用弱分类器？**
<br>默认一般用一个决策树的树桩(也就是1个root加2个leaf), 由于ada boost算法是二分类算法，
所以弱分类器也可以用logistic reg。
<br>之所以要采用弱分类器做子分类器，是因为？？？

**Q. 随机森林和ada boost在算法上有什么区别？**

- 两种算法都是在多个弱分类器上的增强：一个是并行的多个弱分类器投票，另一个是串行提高错分样本权重，
...
- ada boost属于boosting的思想：在整个数据集上，串行创建多个弱分类器，一方面定义出每个弱分类器自身的权重alpha，预测精度越高则权重越大；
  ...
  也就保证最终的分类器精度超过每个弱分类器精度。如下是一组弱分类器组合成强分类器的数据：
```
