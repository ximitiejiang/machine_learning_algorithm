...
1. 首先定义什么是归一化，什么是标准化：
    - 归一化(normalization): xi' = (xi - xmin) / (xmax - xmin)，也就是把数据变换到[0,1]区间
    - 标准化(standardization): xi' = (xi - mu)/sigma，也就是把数据变换到N(0,1)标准方差区间(可能大于1)
...
**Q. 机器学习如何定义损失函数？**
1. 思路1：基于极大似然估计，损失最小化就是概率最大化，所以找到概率表达式取负号就是loss
- 先写出概率表达式P(y=1|x)=y', P(y=0|x)=1-y'，其中y'为预测概率(即经过sigmoid函数后的概率)
- 再组合表达式得P(y|x) = (y')^y*(1-y')^(1-y)
- 取对数的log(P) = y*log(y') + (1-y)*log(1-y'), 
- 取负数就得到损失函数logistic loss = -(y*log(y') + (1-y)*log(1-y')), 这是常用的二分类和多分类损失函数

2. 思路2：基于熵，损失最小化就是熵最小化，所以求出熵表达式(熵的本质是比较预测值与真实值两个子集的不确定度)
- 决策树中经验熵H = - sum(pk*log(pk)), 其中pk为分割后的概率[0~1]
- 深度学习中交叉熵H = -(y*log(y') + (1-y)*log(1-y'))，其中y为标签[0,1]， y'为概率[0~1]
- 以上两个熵其实是等价的，并且交叉熵的公式跟logistic loss的公式是完全一样的。
...
5. ROC曲线和AUC：ROC曲线叫受试者特征曲线，用于对召回率和精确率进行可视化，其中x轴为FP_rate, y轴为TP_rate，
   ROC曲线跟x轴的面积就是AUC，用于评估分类模型总体性能(类似F1的功能)
