...
模型名称     |分类   |特征    |优点                            | 缺点 
 -           |:-     |:-     |:-                              |:-
knn          |多分类 |非线性 |支持几乎所有类型数据，结构简单   |不是最优解，分隔面会侵入少样本区，计算量大
logistic reg |二分类 |线性   |                                |
softmax reg  |多分类 |线性   |支持线性可分的多分类             |不是最优解，只支持线性分隔面
perceptron   |二分类 |线性   |                                |
svm          |多分类 |非线性 |支持几乎所有类型数据             |完美!
naive bayes  |多分类 |非线性 |支持连续和离散数据               |非线性能力不太强，对连续特征要求是正态分布否则分类效果差
